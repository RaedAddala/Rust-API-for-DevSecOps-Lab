...
use axum::{extract::DefaultBodyLimit, http::HeaderValue, middleware, routing::{ get, post }, Router};
use axum::http::header::STRICT_TRANSPORT_SECURITY;
use sqlx::{ postgres::PgPoolOptions, PgPool };
use tower_http::{compression::CompressionLayer, cors::{ Any, CorsLayer }, request_id::{ MakeRequestUuid, PropagateRequestIdLayer, SetRequestIdLayer }, trace::TraceLayer};
use tracing::{ error, info, warn };

use library_api::{ build_rate_limiter, handlers, middleware_auth, AppState, Config };
...
async fn main() -> Result<()> {
    ...
    let db = connect_with_retry(&config.database_url).await.map_err(|e|
        anyhow!("Failed to connect to PostgreSQL after retries: {e}")
    )?;
    ...
    let protected_routes = Router::new()
        ...
        .route_layer(middleware::from_fn_with_state(state.clone(), middleware_auth::auth_middleware));

    let hsts_value: HeaderValue = HeaderValue::from_static(
